...
###PoweStatur 1on 0off
set theComandText to ("Ä"" & theBinPath & "Ä"  --paired") as text
...
repeat with objDevices in listPaiedDevices
	...
	set AppleScript's text item delimiters to ""
	try
		set theComandText to ("Ä"" & theBinPath & "Ä" --connect Ä"" & theDevicesAdd & "Ä"") as text
		do shell script theComandText
	on error
		log "connectÉGÉâÅ["
	end try
	try
		set theComandText to ("Ä"" & theBinPath & "Ä" --add-favourite Ä"" & theDevicesAdd & "Ä"") as text
		do shell script theComandText
	on error
		log "add-favouriteÉGÉâÅ["
	end try
