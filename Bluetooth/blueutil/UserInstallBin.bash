...
############################################################
###バージョンチェック
STR_OLDVER_PATH="/Users/$CONSOLE_USER/bin/blueutil/blueutil"
if [[ -e "$STR_OLDVER_PATH" ]]; then
STR_VER=$("$STR_OLDVER_PATH" -v)
/bin/echo "STR_VER:$STR_VER"
fi
...
###ダウンロードURL
STR_URL="https://github.com/toy/blueutil/archive/refs/tags/v2.9.1.tar.gz"
###ファイル名を取得
...
/bin/echo "ダウンロードOK"
/bin/mkdir -p "$USER_TEMP_DIR/blueutil/"
############################################################
######### インストール
/bin/echo "インストール開始:" "$CONSOLE_USER"
####解凍
/usr/bin/bsdtar -xzf "$USER_TEMP_DIR/$DL_FILE_NAME" -C "$USER_TEMP_DIR/blueutil" --strip-components=1
sleep 2
/bin/echo "カレントディレクトリ:$USER_TEMP_DIR/blueutil"
cd "$USER_TEMP_DIR/blueutil" || return
cd "$USER_TEMP_DIR/blueutil" && /usr/bin/make
####移動
/usr/bin/ditto "$USER_TEMP_DIR/blueutil" "/Users/$CONSOLE_USER/bin/blueutil"
####終了
