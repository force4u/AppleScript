...
set strAppendPath to ("com.apple.sharedfilelist/com.apple.LSSharedFileList.ProjectsItems.sfl2") as text
set ocidFilePathURL to ocidAppSuppDirPathURL's URLByAppendingPathComponent:(strAppendPath) isDirectory:false
set ocidSfl2FileName to ocidFilePathURL's lastPathComponent()
###保存先
set appFileManager to refMe's NSFileManager's defaultManager()
set ocidURLsArray to (appFileManager's URLsForDirectory:(refMe's NSDesktopDirectory) inDomains:(refMe's NSUserDomainMask))
set ocidDesktopDirPathURL to ocidURLsArray's firstObject()
set ocidSavesfl2FilePathURL to ocidDesktopDirPathURL's URLByAppendingPathComponent:(ocidSfl2FileName)
set ocidSaveFilePathURL to ocidSavesfl2FilePathURL's URLByAppendingPathExtension:("plist")
...
repeat with itemsArrayDict in ocidItemsArray
	...
	if ocidBookMarkURL is not (missing value) then
		...
		set strBoolMarkSpec to coidAliasStrings's stringByRemovingPercentEncoding()
		###値をセットする
		if (ocidLocaleID as text) contains "ja" then
			(ocidAddDictM's setValue:(coidAliasStrings) forKey:("Bookmarkのテキスト表示"))
			(ocidAddDictM's setValue:(strBoolMarkSpec) forKey:("Bookmarkのテキスト表示デコード済み"))
		else
