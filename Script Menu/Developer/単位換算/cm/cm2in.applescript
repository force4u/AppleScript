...
set strMes to ("CM数を半角数字で入力") as text
...
###そのまま
set numRaw to (strResponse / numVar) as number
set strMes to ("計算結果です:" & strResponse & "Cm →" & numRaw & "in\r") as text
log numRaw
###整数切り捨て
...
####
set strMes to (strMes & "小数点以下計算は切り捨て") as text
...
try
	set recordResult to (display dialog strMes with title strMes default answer numRaw buttons {"クリップボードにコピー", "終了", "再実行"} default button "再実行" cancel button "終了" giving up after 20 with icon aliasIconPath without hidden answer) as record
on error
	log "エラーしました"
end try
