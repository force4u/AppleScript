...
#日本円の逆算用
set listAddPairCode to {"JPYAUD", "JPYCAD", "JPYCHF", "JPYEUR", "JPYGBP", "JPYNZD", "JPYUSD", "JPYZAR"} as list
set ocidAddPairCode to refMe's NSArray's arrayWithArray:(listAddPairCode)
ocidDialogueArrayM's addObjectsFromArray:(ocidAddPairCode)
...
try
	set listResponse to (choose from list listDialogueArrayM with title "選んでください" with prompt "ペアコードを選んでください" default items (item 25 of listDialogueArrayM) OK button name "OK" cancel button name "キャンセル" without multiple selections allowed and empty selection allowed) as list
on error
	log "エラーしました"
	return "エラーしました"
end try
...
repeat with itemJsonArrayM in ocidJsonArrayM
	...
	set ocidRange to refMe's NSMakeRange(0, 3)
	set ocidGetRengeStr to (ocidResponse's substringWithRange:(ocidRange))
	
	if (ocidGetRengeStr as text) is "JPY" then
		set ocidPairCode to (refMe's NSMutableString's stringWithCapacity:(6))
		set ocidRange to refMe's NSMakeRange(3, 3)
		set ocidGetRengeEndStr to (ocidResponse's substringWithRange:(ocidRange))
		(ocidPairCode's appendString:(ocidGetRengeEndStr))
		(ocidPairCode's appendString:(ocidGetRengeStr))
		log strPairCode
		log ocidPairCode as text
		if strPairCode is (ocidPairCode as text) then
			set strlow to (ocidQuotesDict's valueForKey:("low"))
			set strhigh to (ocidQuotesDict's valueForKey:("high"))
			set stropen to (ocidQuotesDict's valueForKey:("open"))
			set strbid to (ocidQuotesDict's valueForKey:("bid"))
			...
			set numhigh to (strhigh as text) as number
			set numNewYen to (numhigh / numImput) as number
			set strNowYen to (numNewYen * strReturnedText) as text
		end if
	end if
end repeat
