...
set strReturnedTextMM to strOne of dialogResult
set strReturnedTextPPI to strTwo of dialogResult

##############################
#####戻り値整形
##############################
set ocidResponseText to (refMe's NSString's stringWithString:(strReturnedTextMM))
###タブと改行を除去しておく
...
###数値に
set strReturnedTextMM to strTextM as number
...
###そのまま
set numRaw to (strReturnedTextMM / (numVar / strReturnedTextPPI)) as number
set strMes to ("計算結果です:" & strReturnedTextMM & "mm →" & numRaw & "px(" & strReturnedTextPPI & "ppi)　　\r") as text
log numRaw
###整数切り捨て
...
on doPerformDialog:(args)
	set strOneLabel to refMe's NSTextField's labelWithString:("mm size:")
	...
	strOneTextField's setPlaceholderString:("mmサイズ")
	...
	strTwoTextField's setPlaceholderString:("解像度")
	...
	appDialogWindow's setTitle:"mm→PX数(解像度指定)"
