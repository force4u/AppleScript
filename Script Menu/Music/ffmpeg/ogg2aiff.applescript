...
set strPathToFFMPEG to (do shell script strPathToFFMPEG) as text
#
...
set listAliasFile to (choose file "org.xiph.ogg-audio ファイルを選んでください" with prompt "ファイルを選んでください" default location (path to desktop folder from user domain) of type {"org.xiph.ogg-audio"} with invisibles, showing package contents and multiple selections allowed) as list
...
repeat with itemAliasFile in listAliasFile
	...
	##保存ファイル名
	set strSaveFileName to (strBaeFileFileName & "." & strDateNo & ".aiff") as text
	##保存URL
...
-codec:a  aac  -ar 32K -ab 128k -aac_coder twoloop
	*)
	...
	##mp3の場合
	#set strCommandText to ("\"" & strPathToFFMPEG & "\" -i '" & strFilePath & "' -codec:a  libmp3lame  -r:a 44100  -b:a 320k '" & strSaveFilePath & "'") as text
	
	set strCommandText to ("\"" & strPathToFFMPEG & "\" -i '" & strFilePath & "'  '" & strSaveFilePath & "'") as text
