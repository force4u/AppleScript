...
repeat with itemAliasFile in listAliasFile
	...
	##保存ファイル名
	set strSaveFileName to (strBaeFileFileName & "." & strDateNo & ".m4a") as text
	##保存URL
...
-strict experimental 
	*)
	...
	##mp3の場合
	set strCommandText to ("\"" & strPathToFFMPEG & "\" -i '" & strFilePath & "' -codec:a  aac  -r:a 44100  -b:a 320k '" & strSaveFilePath & "'") as text
