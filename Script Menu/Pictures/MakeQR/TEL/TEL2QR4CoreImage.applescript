...
if (count of listTelNo) = 0 then
	set strDefaultAnswer to "080"
else
	set strDefaultAnswer to (item 1 of listTelNo) as text
end if
#####################
###ダイアログ
...
try
	set objResponse to (display dialog "電番を入力してください" with title "電用のQRコードを作成します" default answer strDefaultAnswer buttons {"OK", "キャンセル"} default button "OK" cancel button "キャンセル" with icon aliasIconPath giving up after 30 without hidden answer)
on error
	log "エラーしました"
	return
end try
...
set strSaveFileName to ("TEL." & strTelNo & "." & strDateNo & ".png") as text
set ocidSaveFilePathURL to ocidSaveDirPathURL's URLByAppendingPathComponent:(strSaveFileName)
#####################
###SMSTOメッセージ
set strSMSTO to ("TEL:" & strTelNo & "") as text
...
############################
####テキストをNSStringに
set ocidInputString to refMe's NSString's stringWithString:(strSMSTO)
####テキストをUTF8に
