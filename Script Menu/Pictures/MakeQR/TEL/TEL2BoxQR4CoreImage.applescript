...
try
	set objResponse to (display dialog "電話番号（数字だけ）を入力してください" with title "QRコードを作成します" default answer strDefaultAnswer buttons {"OK", "キャンセル"} default button "OK" cancel button "キャンセル" with icon aliasIconPath giving up after 30 without hidden answer)
on error
	log "エラーしました"
	return
end try
...
##数字のみのCharset
set ocidNumCharSet to refMe's NSCharacterSet's decimalDigitCharacterSet()
set ocidNoNumCharSet to ocidNumCharSet's invertedSet()
set ocidRange to ocidTextM's rangeOfCharacterFromSet:(ocidNoNumCharSet)
set ocidLocation to ocidRange's location
set strText to ocidTextM as text
if ocidLocation = refNSNotFound then
	set strQRContents to ("TEL:" & strText) as text
else
	return "【エラー】数字以外が含まれています"
end if
...
(refMe's NSGraphicsContext's setCurrentContext:(ocidContext))
##############
set ocidText to (refMe's NSString's stringWithString:("TEL QR"))
...
*)
set strIconText to (character id 1052805) as text
...
(ocidTextAttr's setObject:(ocidFont) forKey:(refMe's NSFontAttributeName))
(ocidTextAttr's setObject:(0) forKey:(refMe's NSKernAttributeName))
set ocidTextOrigin to refMe's NSMakePoint((390), (20))
(ocidText's drawAtPoint:(ocidTextOrigin) withAttributes:(ocidTextAttr))
...
set ocidSaveDirPathURL to ocidPicturesDirURL's URLByAppendingPathComponent:("QRcode/TEL")
##フォルダ作成
...
set strSaveFileName to ("TEL" & "." & strText & "." & strDateNo & ".png") as text
