...
repeat with itemChooseFiles in listChooseFiles
	...
	set strSaveFileName to (strBaseFileName & "." & strPixelsWidth & "x" & strPixelsHeight & ".png") as text
	######
	...
	####NSImageGamma
	(ocidProperties's setValue:(1.0) forKey:(refMe's NSImageGamma))
	####NSImageInterlaced
	(ocidProperties's setValue:(refMe's NSNumber's numberWithBool:false) forKey:(refMe's NSImageInterlaced))
	#####JPEGデータに変換
	set ocidSaveData to (ocidReadImageRep's representationUsingType:(refMe's NSBitmapImageFileTypePNG) |properties|:(ocidProperties))
	###保存
