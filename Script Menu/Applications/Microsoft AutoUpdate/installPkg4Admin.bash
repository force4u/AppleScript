...
#################################
###ここだけ変えればなんでいけるか？
STR_URL="https://go.microsoft.com/fwlink/?linkid=830196"
###
STR_APP_NAME="Microsoft AutoUpdate"
...
if [ -z "$CONSOLE_USER" ]; then
	...
	if ! /usr/bin/osascript -e "tell application id \"com.microsoft.autoupdate2\" to quit"; then
		##念の為　KILLもする
...
function DO_MOVE_TO_TRASH_SUDO() {
	if [ -e "$1" ]; then
		TRASH_DIR=$(/usr/bin/sudo -u "$CONSOLE_USER" /usr/bin/mktemp -d "/Users/$CONSOLE_USER/.Trash/Autoupdate2.XXXXXXXX")
...
function DO_MOVE_TO_TRASH_USER() {
	...
	if [ -e "$STR_DO_FILE_PATH" ]; then
		USER_TRASH_DIR=$(/usr/bin/sudo -u "$STR_DO_USERID" /usr/bin/mktemp -d "/private/tmp/Cleanup At Startup/.Trash/$STR_DO_USERID/Autoupdate2.XXXXXXXX")
