...
repeat with itemKey in listKeyList
	...
	try
		set theComand to ("/usr/sbin/cupsenable \"" & strDeviceName & "\"") as text
		set theLog to (do shell script theComand) as text
		log theLog
	end try
	try
		set theComand to ("/usr/sbin/cupsaccept \"" & strDeviceName & "\"") as text
		set theLog to (do shell script theComand) as text
		log theLog
	end try
end repeat
...
repeat with objPrinterName in listPrinterList
	log objPrinterName
	try
		set theComand to ("/usr/sbin/cupsenable \"" & objPrinterName & "\"") as text
		set theLog to (do shell script theComand) as text
		log theLog
	end try
	try
		set theComand to ("/usr/sbin/cupsaccept \"" & objPrinterName & "\"") as text
		set theLog to (do shell script theComand) as text
		log theLog
	end try
end repeat
