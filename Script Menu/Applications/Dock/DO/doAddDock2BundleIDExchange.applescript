...
## zero-based indexing ポジション
##Dockのこのポジションに登録　または移動
set numPosIndex to (5) as integer
...
if numTileIndex ≠ (missing value) then
	...
	#	ocidPersistentAppsArray's insertObject:(ocidTileAppDict) atIndex:(numTileIndex)
	ocidPersistentAppsArray's exchangeObjectAtIndex:(numTileIndex) withObjectAtIndex:(numPosIndex)
else
	###　ないなら作る
	set ocidTileAppDict to doGetPersistentAppsDict(strBundleID)
