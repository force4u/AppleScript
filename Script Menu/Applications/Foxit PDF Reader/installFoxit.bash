...
#################################
###ここだけ変えればなんでいけるか？
STR_URL="https://www.foxit.com/downloads/latest.html?product=Foxit-Reader&platform=Mac-OS-X&version=&package_type=&language=English&distID="
###
STR_APP_NAME="Foxit PDF Reader"
...
if [ -z "$CONSOLE_USER" ]; then
	...
	if ! /usr/bin/osascript -e "tell application id \"com.foxit-software.Foxit PDF Reader\" to quit"; then
		##念の為　KILLもする
		/usr/bin/killall "$STR_APP_NAME" 2>/dev/null
		/usr/bin/killall "FoxitPDFReaderUpdateService" 2>/dev/null
...
###Dockに入れない場合はコメントアウト
/usr/bin/sudo -u "$SUDO_USER" /usr/bin/defaults  write com.apple.dock persistent-apps -array-add "<dict><key>tile-data</key><dict><key>file-data</key><dict><key>_CFURLString</key><string>/Applications/$STR_APP_NAME.app/</string><key>_CFURLStringType</key><integer>0</integer></dict></dict></dict>"
