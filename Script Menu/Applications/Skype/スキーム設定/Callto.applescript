...
###設定項目 URLスキーム
set strScheme to "callto://" as text
###NSURL
...
try
	set listResponse to (choose from list listAppName with title "選んでください" with prompt "URLスキーム\r\r【" & strScheme & "】を開く\rアプリケーションを選んでください" default items (item 1 of listAppName) OK button name "OK" cancel button name "キャンセル" without multiple selections allowed and empty selection allowed)
on error
	log "エラーしました"
	return "エラーしました"
end try
...
################################
set appShardWorkspace to refMe's NSWorkspace's sharedWorkspace()
appShardWorkspace's setDefaultApplicationAtURL:(ocidAppPathURL) toOpenURLsWithScheme:("callto") completionHandler:(missing value)
