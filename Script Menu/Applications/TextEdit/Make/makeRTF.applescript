...
if boolDirExists is true then
	...
	set strFileName to "名称未設定." & strDateno & ".rtf"
	set ocidSaveFilePathURL to ocidUserDesktopPathURL's URLByAppendingPathComponent:(strFileName) isDirectory:false
end if
set strFullFilePath to ocidSaveFilePathURL's |path|() as text

tell application "TextEdit"
	activate
	make new document with properties {name:strFileName}
	tell document strFileName
		activate
		save in (POSIX file strFullFilePath)
	end tell
end tell
