...
set theResponse to "297x210"
try
	set objResponse to (display dialog "書類サイズを幅x高さで入力" with title "数値入力" default answer theResponse buttons {"OK", "キャンセル"} default button "OK" cancel button "キャンセル" with icon aliasIconPath giving up after 30 without hidden answer)
on error
	log "エラーしました"
	return
end try
...
set numPtHight to round (numPtHight * 2.835) as number
set numPtWidth to round (numPtWidth * 2.835) as number
