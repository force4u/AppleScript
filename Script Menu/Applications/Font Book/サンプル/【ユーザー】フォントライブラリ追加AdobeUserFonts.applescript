...
set strDefaultName to "AdobeUserFonts.library" as text
...
set strFontsDirPath to "~/Library/Application Support/Adobe/.User Owned Fonts"
...
repeat with itemAliasFontPath in ocidFontPathArray
	set listBookMarkData to (itemAliasFontPath's bookmarkDataWithOptions:(11) includingResourceValuesForKeys:({missing value}) relativeToURL:(missing value) |error|:(reference))
	set ocidBookMarkData to item 1 of listBookMarkData
	set strPath to itemAliasFontPath's |path|() as text
	(ocidBookmarks's setObject:(ocidBookMarkData) forKey:(strPath))
end repeat
