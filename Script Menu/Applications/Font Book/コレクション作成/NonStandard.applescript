...
####設定項目
set strTraitsName to ("NonStandard") as text
set ocidTraits to (refMe's NSNonStandardCharacterSetFontMask)
####スタイルリスト
set listFontFace to {"Poster", "Pinpoint", "Outline", "Hatched", "Deco", "Color", "Flourish", "FutoLine", "Fractions", "Inline", "Shadow", "Solid","Inclined","Black and White"} as list
#Arrayにしておく
set ocidFontFaceArray to refMe's NSArray's alloc()'s initWithArray:(listFontFace)
####曖昧比較用リスト
set listFontFaceAmb to {"Poster", "Pinpoint", "Outline", "Hatched", "Deco", "Color", "Flourish", "FutoLine", "Fractions", "Inline", "Shadow", "Solid"} as list
###特定フォント名からリストに追加用
set listContainFontName to {"emoji", "Symbol", "Ornaments", "Hand", "Script"} as list
...
repeat with itemFont in ocidFontArray
	...
	repeat with itemContainName in listContainFontName
		if (ocidFontName as text) contains itemContainName then
			set boolSetFontDiscriptor to true as boolean
		end if
	end repeat
	#########NSFontDescriptor
	set ocidItemFontDiscriptor to ocidItemFont's fontDescriptor()
	#set ocidSymbolTraits to ocidItemFontDiscriptor's symbolicTraits()
	#log (ocidSymbolTraits)
	#スタイルを取得して
	set ocidItemFontFace to (ocidItemFontDiscriptor's objectForKey:(refMe's NSFontFaceAttribute))
	log ocidItemFontFace as text
	set ocidItemFontTrait to (ocidItemFontDiscriptor's objectForKey:(refMe's NSFontWidthTrait))
	log ocidItemFontTrait as text
	#set ocidItemFontAttr to (ocidItemFontDiscriptor's objectForKey:(refMe's NSFontTraitsAttribute))
	#	set ocidAttrAllKey to ocidItemFontAttr's allKeys()
	#log ocidAttrAllKey as list
	#リストに含まれているか？
