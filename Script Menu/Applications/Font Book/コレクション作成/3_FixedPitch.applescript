...
----+----1----+----2----+-----3----+----4----+----5----+----6----+----7
#
# モノスペース FixedPitchのフォントのコレクションを作成
...
####設定項目
set strTraitsName to ("FixedPitch") as text
set ocidTraits to (refMe's NSFixedPitchFontMask)
...
repeat with itemFont in ocidFontArray
	...
	#isFixedPitch　モノスペースか？
	set boolMonoSpase to ocidItemFont's isFixedPitch() as boolean
	if boolMonoSpase is true then
