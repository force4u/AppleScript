...
----+----1----+----2----+-----3----+----4----+----5----+----6----+----7
#
# 書体のスタイルが　Regular　Normal　Mediumの書体の
#コレクションをRegularで登録する
# 『斜体』を登録しない方法
...
####設定項目
set strTraitsName to ("Regular") as text
set ocidTraits to (refMe's NSUnboldFontMask)
####スタイルリスト
set listFontFace to {"Regular", "Normal", "Medium", "Plain", "Text", "M", "R", "N", "Roman", "400", "500", "600", "W4", "W5", "W6", "Mittelschrift", "Regulär", "Standard"} as list
#Arrayにしておく
set ocidFontFaceArray to refMe's NSArray's alloc()'s initWithArray:(listFontFace)
####曖昧比較用リスト
set listFontFaceAmb to {"Regular", "Normal", "Medium", " R", " M", "Md"} as list
	...
	if boolContain is true then
		...
		repeat with itemFontFaceAmb in listFontFaceAmb
			...
			if (ocidRange's location()) ≠ (refNotFound) then
				...
				set ocidRangeElm to (ocidItemFontFace's rangeOfString:("Oblique") options:(refMe's NSCaseInsensitiveSearch))
				if (ocidRangeElm's location()) = (refNotFound) then
					set ocidRangeElm to (ocidItemFontFace's rangeOfString:("Italic") options:(refMe's NSCaseInsensitiveSearch))
					if (ocidRangeElm's location()) = (refNotFound) then
						#登録して良い判断
						set boolSetFontDiscriptor to true as boolean
					end if
