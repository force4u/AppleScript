...
####設定項目
set strTraitsName to ("Bold") as text
set ocidTraits to (refMe's NSBoldFontMask)
####スタイルリスト
set listFontFace to {"Bold", "Black", "Demibold", "Semibold", "H", "U", "D", "B", "Extrabold", "ExtraBlack", "BD", "DB", "UB", "EB", "Heavy", "Blk", "Fat", "Hv", "700", "800", "900", "1000", "UltraBlack", "UltraBold", "W7", "W8", "W9", "W10"} as list
#Arrayにしておく
set ocidFontFaceArray to refMe's NSArray's alloc()'s initWithArray:(listFontFace)
####曖昧比較用リスト
set listFontFaceAmb to {"Bold", "Black", "Demi", "Heavy", "Semi", "Blk", "Bk","Dm", " B", " H"} as list
...
####
set strSetUnTraitsName to ("Unbold") as text
set ocidUnBaseFilePathURL to ocidSaveDirPathURL's URLByAppendingPathComponent:(strSetUnTraitsName) isDirectory:(false)
set ocidUnSaveFilePathURL to ocidUnBaseFilePathURL's URLByAppendingPathExtension:("collection")
#ファイル名
set ocidFileName to ocidSaveFilePathURL's lastPathComponent()
set ocidUnFileName to ocidUnSaveFilePathURL's lastPathComponent()
################################
#保存するDICTの初期化
set ocidPlistDict to refMe's NSMutableDictionary's alloc()'s initWithCapacity:(0)
set ocidUnPlistDict to refMe's NSMutableDictionary's alloc()'s initWithCapacity:(0)
...
#
set ocidSetUnCollectionDict to refMe's NSMutableDictionary's alloc()'s initWithCapacity:(0)
ocidSetUnCollectionDict's setValue:(strSetUnTraitsName) forKey:("NSFontCollectionName")
ocidSetUnCollectionDict's setValue:(ocidUnFileName) forKey:("NSFontCollectionFileName")
ocidUnPlistDict's setObject:(ocidSetUnCollectionDict) forKey:("NSFontCollectionAttributes")

##【２】NSFontCollectionFontDescriptors
set ocidSetDescriptorsArray to refMe's NSMutableArray's alloc()'s initWithCapacity:(0)
set ocidSetUnDescriptorsArray to refMe's NSMutableArray's alloc()'s initWithCapacity:(0)
##NSFontManager
...
repeat with itemFont in ocidFontArray
	...
	else if boolSetFontDiscriptor is false then
		(ocidSetUnDescriptorsArray's addObject:(ocidItemFontDiscriptor))
	end if
end repeat
ocidPlistDict's setObject:(ocidSetDescriptorsArray) forKey:("NSFontCollectionFontDescriptors")
ocidUnPlistDict's setObject:(ocidSetUnDescriptorsArray) forKey:("NSFontCollectionFontDescriptors")
...
################################
#
set ocidArchiverDataUn to refMe's NSKeyedArchiver's alloc()'s initRequiringSecureCoding:(false)
#エンコード
ocidArchiverDataUn's encodeObject:(ocidUnPlistDict) forKey:("NSFontCollectionDictionary")
ocidArchiverDataUn's finishEncoding()
set ocidEncDataUn to ocidArchiverDataUn's encodedData()
#保存するDATA
set ocidSaveDataUn to refMe's NSMutableData's alloc()'s initWithCapacity:(0)
ocidSaveDataUn's appendData:(ocidEncDataUn)
...
################################
#保存
set listDone to ocidSaveDataUn's writeToURL:(ocidUnSaveFilePathURL) options:(ocidOption) |error|:(reference)
if (item 2 of listDone) = (missing value) then
	log "writeToURL　正常処理"
else if (item 2 of listDone) ≠ (missing value) then
	log (item 2 of listDone)'s code() as text
