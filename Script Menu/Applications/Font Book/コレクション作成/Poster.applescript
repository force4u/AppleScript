...
####設定項目
set strTraitsName to ("Poster") as text
set ocidTraits to (refMe's NSPosterFontMask)
####スタイルリスト
set listFontFace to {"Poster"} as list
#Arrayにしておく
set ocidFontFaceArray to refMe's NSArray's alloc()'s initWithArray:(listFontFace)
####曖昧比較用リスト
set listFontFaceAmb to {"Poster"} as list
...
repeat with itemFont in ocidFontArray
	...
	if (ocidFontName as text) contains "Poster" then
