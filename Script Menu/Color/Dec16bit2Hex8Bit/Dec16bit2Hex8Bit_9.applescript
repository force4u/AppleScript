...
on do16bitDec2Hex(list16BitDec)
	...
	#	set strPyScript to ("dec_array = [" & strDec8BitR & ", " & strDec8BitG & ", " & strDec8BitB & "];hex_string = dec_array.pack(\"C*\").unpack(\"H*\").first.upcase;puts hex_string;") as text
	set strPyScript to ("puts [" & strDec8BitR & ", " & strDec8BitG & ", " & strDec8BitB & "].pack(\"C*\").unpack(\"H*\").first.upcase") as text
	set strHexValue to (do shell script "/usr/bin/ruby -e '" & strPyScript & "'") as text
	return strHexValue
end do16bitDec2Hex
