...
on do16bitDec2Hex(list16BitDec)
	...
	set strPyScript to ("dec_array = [" & strDec8BitR & ", " & strDec8BitG & ", " & strDec8BitB & "];byte_data = bytes(dec_array);hex_string = byte_data.hex().upper();print(hex_string);") as text
	set strHexValue to (do shell script "/usr/bin/python3 -c \"" & strPyScript & "\"") as text
	return strHexValue
end do16bitDec2Hex
