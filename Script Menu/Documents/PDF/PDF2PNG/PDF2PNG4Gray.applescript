...
set strIccFilePath to "/System/Library/ColorSync/Profiles/Generic Gray Gamma 2.2 Profile.icc"
...
repeat with itemPageNo from 0 to (numPageCnt - 1) by 1
	...
	set ocidAardboardRep to (refMe's NSBitmapImageRep's alloc()'s initWithBitmapDataPlanes:(missing value) pixelsWide:(numPxWidth) pixelsHigh:(numPxHeigh) bitsPerSample:(8) samplesPerPixel:(1) hasAlpha:(false) isPlanar:(false) colorSpaceName:(refMe's NSCalibratedWhiteColorSpace) bitmapFormat:(refMe's NSBitmapFormatAlphaNonpremultiplied) bytesPerRow:(0) bitsPerPixel:(0))
	#
	...
	(ocidPropertyDict's setValue:(refMe's NSNumber's numberWithDouble:(2.2)) forKey:(refMe's NSImageGamma))
	(ocidPropertyDict's setValue:(refMe's NSNumber's numberWithBool:false) forKey:(refMe's NSImageInterlaced))
