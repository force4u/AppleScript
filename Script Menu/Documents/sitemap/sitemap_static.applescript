...
### 設定項目
###ベースURL
set strBaseURL to ("https://foo.hoge.com/contents/") as text
###サブパス
set listURL to {"main", "owners"} as list
...
set ocidContainerFilePathURL to ocidDesktopDirPathURL's URLByAppendingPathComponent:("sitemap_static.xml") isDirectory:false
###【A】ROOT エレメント
set ocidRootElement to refMe's NSXMLElement's alloc()'s initWithName:"urlset"
...
repeat with itemURL in listURL
	set strURL to (strBaseURL & itemURL) as text
	###【B】子要素　sitemap　エレメント
	set ocidSitemapElement to (refMe's NSXMLElement's alloc()'s initWithName:("url"))
	
	###【C】Bの子要素としてのloc　エレメント
	set ocidLocElement to (refMe's NSXMLElement's elementWithName:("loc"))
	###【C】にテキストの値を設定
	(ocidLocElement's setStringValue:(strURL))
