...
try
	set strMes to ("SSHサーバーのURLを入力してください\n例：\nssh://guest:guest@foo.hoge.com\nssh://USERID:PASSWORD@192.168.0.0\nssh://localhost") as text
...
if strResponse starts with "ssh:" then
...
set ocidSaveDirPathURL to ocidDocumentDirPathURL's URLByAppendingPathComponent:("Apple/Webloc/SSH/") isDirectory:(true)
##ディレクトリ作成
set appFileManager to refMe's NSFileManager's defaultManager()
