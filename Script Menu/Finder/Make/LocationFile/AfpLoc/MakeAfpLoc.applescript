...
if strDefaultAnswer starts with "afp:" then
	set strURL to strDefaultAnswer as text
else
	set strURL to ("afp://") as text
end if
...
try
	set strMes to ("AFPサーバーのURLを入力してください\r例：\rafp://;AUTH=No%20User%20Authent@foo.hoge.com/Public\nafp://USERID:PASSWORD@192.168.0.0/Public\nafp://192.168.0.0/Public") as text
	
	set recordResponse to (display dialog strMes with title "AFPのURLを入力してください" default answer strURL buttons {"OK", "キャンセル"} default button "OK" cancel button "キャンセル" with icon aliasIconPath giving up after 20 without hidden answer)
on error
	log "エラーしました"
	return "エラーしました"
end try
...
if strResponse starts with "afp:" then
...
##ファイル名
set strFileName to (ocidHostName & ".afploc") as text
...
set ocidSaveDirPathURL to ocidDocumentDirPathURL's URLByAppendingPathComponent:("Apple/Webloc/AFP/") isDirectory:(true)
##ディレクトリ作成
set appFileManager to refMe's NSFileManager's defaultManager()
