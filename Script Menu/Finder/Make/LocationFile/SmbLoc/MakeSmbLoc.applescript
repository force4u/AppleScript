...
if strDefaultAnswer starts with "cifs:" then
	set strURL to strDefaultAnswer as text
else if strDefaultAnswer starts with "smb:" then
	set strURL to strDefaultAnswer as text
else
	set strURL to ("smb://") as text
end if
...
try
	set strMes to ("SMBサーバーのURLを入力してください\n例：\nsmb://guest:guest@foo.hoge.com/Public\ncifs://USERID:PASSWORD@192.168.0.0/Public\nsmb://192.168.0.0/Public") as text
	
	set recordResponse to (display dialog strMes with title "SMBのURLを入力してください" default answer strURL buttons {"OK", "キャンセル"} default button "OK" cancel button "キャンセル" with icon aliasIconPath giving up after 20 without hidden answer)
on error
	log "エラーしました"
	return "エラーしました"
end try
...
if strResponse starts with "smb:" then
	set strURL to strResponse as text
else if strResponse starts with "cif:" then
	set strURL to strResponse as text
else
	return "SMBのURL専用です"
end if
...
set ocidSaveDirPathURL to ocidDocumentDirPathURL's URLByAppendingPathComponent:("Apple/Webloc/SMB/") isDirectory:(true)
##ディレクトリ作成
set appFileManager to refMe's NSFileManager's defaultManager()
