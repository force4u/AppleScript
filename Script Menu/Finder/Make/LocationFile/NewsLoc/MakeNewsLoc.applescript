...
#
(*
Usenet  newsリーダーはこちらから
https://blog.panic.com/ja/the-future-of-unison-j/
*)
...
if strDefaultAnswer starts with "news:" then
	set strURL to strDefaultAnswer as text
else
	set strURL to ("news://") as text
end if
...
try
	set strMes to ("NEWSサーバーのURLを入力してください\r例：\rnews://news.neodome.net/uk.comp.sys.mac/") as text
	
	set recordResponse to (display dialog strMes with title "NEWSのURLを入力してください" default answer strURL buttons {"OK", "キャンセル"} default button "OK" cancel button "キャンセル" with icon aliasIconPath giving up after 20 without hidden answer)
on error
	log "エラーしました"
	return "エラーしました"
end try
...
if strResponse starts with "news:" then
...
##ファイル名
set strFileName to (ocidHostName & ".newsloc") as text
...
set ocidSaveDirPathURL to ocidDocumentDirPathURL's URLByAppendingPathComponent:("Apple/Webloc/News/") isDirectory:(true)
##ディレクトリ作成
set appFileManager to refMe's NSFileManager's defaultManager()
