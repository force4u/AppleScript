...
##############################################
try
	set boolCreateDesktop to (do shell script "/usr/bin/defaults read  com.apple.finder CreateDesktop") as integer
on error
...
if boolCreateDesktop = 1 then
	--->0=FALSEに変える
	try
		do shell script "/usr/bin/defaults write  com.apple.finder CreateDesktop -bool false"
	on error
	...
	--->0なら--> 1=trueに変更する
	try
		do shell script "/usr/bin/defaults write  com.apple.finder CreateDesktop -bool true"
	on error
		log "trueに変更できませんでした"
	end try
end if
