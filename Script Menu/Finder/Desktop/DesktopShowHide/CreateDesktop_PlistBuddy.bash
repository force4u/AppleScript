...
#PlistBuddy
STR_BOOL_VALUE=$(/usr/libexec/PlistBuddy -c "Print:CreateDesktop:"  "$STR_PLIST_PATH")
/bin/echo "PlistBuddy:ユーザー設定は $STR_BOOL_VALUE　です"

if [ "$STR_BOOL_VALUE" = "true" ]; then
	/usr/libexec/PlistBuddy -c "Set:CreateDesktop false" "$STR_PLIST_PATH"
else
	/usr/libexec/PlistBuddy -c "Set:CreateDesktop true" "$STR_PLIST_PATH"
fi
