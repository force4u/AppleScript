...
##############################################
try
	set boolResponse to (do shell script "/usr/libexec/PlistBuddy -c \"Print:CreateDesktop:\"  \"" & strPlistPath & "\"") as boolean
on error
...
if boolResponse is true then
	--->0=FALSEに変える
	try
		do shell script "/usr/libexec/PlistBuddy -c \"Set:CreateDesktop false\"  \"" & strPlistPath & "\""
	on error
	...
	--->0なら--> 1=trueに変更する
	try
		do shell script "/usr/libexec/PlistBuddy -c \"Set:CreateDesktop true\"  \"" & strPlistPath & "\""
	on error
